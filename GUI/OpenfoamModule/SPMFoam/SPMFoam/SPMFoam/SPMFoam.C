...
Description
    Solver for single particle model of lithium ion batteries.
...
int main(int argc, char *argv[])
{
    ...
    while (pimples.run(runTime))
    {
        ...
        while (pimples.loop())
        {
            ...
            if (flag_SOC == -1) // SOC exceeds its lower limit, which means over-delithiated
            {
              Info<< "Over-delithiated!" << nl << endl;
            }

            if (flag_SOC == 1) // SOC exceeds its higher limit, which means over-lithiated
            {
              Info<< "Over-lithiated!" << nl << endl;
            }

            if (flag_SOC!=0) // if SOC out of normal range
            {
